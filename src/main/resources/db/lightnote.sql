...
create index idx_last_time
    on chat_conversation (last_time);
...
(
    ...
    title       varchar(255)   not null comment '知识标题',
    content     longtext       not null comment '知识正文',
    source_type varchar(32)    null comment '来源类型，例如 shop',
    source_id   bigint         null comment '来源业务ID',
    embedding   longtext       not null comment '向量JSON',
    score       decimal(10, 4) null comment '检索打分缓存，可为空',
    create_time datetime       not null comment '创建时间',
    update_time datetime       not null comment '更新时间'
)
    row_format = DYNAMIC;
...
create index idx_create_time
    on tb_ai_message (create_time);

create index idx_user_id
    on tb_ai_message (user_id);
...
(
    ...
    model_name        varchar(64)                     not null comment '模型名',
    biz_mode          varchar(32)                     not null comment 'chat / agent',
    prompt_tokens     int            default 0        not null comment '输入 token',
    completion_tokens int            default 0        not null comment '输出 token',
    total_tokens      int            default 0        not null comment '总 token',
    estimated_cost    decimal(12, 6) default 0.000000 not null comment '估算成本',
    latency_ms        bigint         default 0        not null comment '耗时毫秒',
    success           tinyint(1)     default 1        not null comment '是否成功',
    error_msg         varchar(512)                    null comment '错误信息',
    prompt_preview    varchar(1000)                   null comment 'prompt 预览',
    create_time       datetime                        not null comment '创建时间'
)
    row_format = DYNAMIC;
